...
void RenderTarget::CreateRenderpass(std::vector<RenderpassAttachment> colorAttachments, RenderpassAttachment* depthAttachment, VkSampleCountFlagBits msaa) {
	if (vulkanRenderpass && ownsRenderpass)
...
void RenderTarget::SetSize(uint32_t sizeX, uint32_t sizeY, bool createColorImage) {
	...
	if (spriteHasBeenCreated && createColorImage) {
		VULKAN::VulkanImageGen::CreateImage(renderedSprite.texture->image.GetPointer(), size, format, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, 1, (VkImageCreateFlagBits)0, 1);
		VULKAN::VulkanImageGen::CreateImageView(renderedSprite.texture->image.GetPointer(), format, VK_IMAGE_ASPECT_COLOR_BIT, VK_IMAGE_VIEW_TYPE_2D, 1, 1);
...
void RenderTarget::Clear(bool complete) {
	...
	if (ownsRenderpass && complete && vulkanRenderpass)
		delete vulkanRenderpass;
}
