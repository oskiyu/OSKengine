...
namespace OSK {
	...
	public:
		...
		ButtonState GetButtonState(const ButtonCode& button) const;


		//Obtiene si un botón está siendo pulsado.
		bool IsButtonDown(const ButtonCode& button) const;
		

		//Obtiene si un botón no está siendo pulsado
		bool IsButtonUp(const ButtonCode& button) const;


		Vector4 GetMouseRectangle(const float_t& size = 2.0f) const;
