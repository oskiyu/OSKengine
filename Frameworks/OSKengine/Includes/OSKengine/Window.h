...
namespace OSK::IO {
	...
	public:
		...
		/// </summary>
		/// 
		/// @pre El identificador debe ser 0 <= identifier <= 3.
		/// 
		/// @throws std::runtime_error si no cumple la precondición.
		const GamepadState& GetGamepadState(TSize identifier) const;
		...
		/// 
		/// @throws std::runtime_error si no cumple la precondición.
		const GamepadState& GetPreviousGamepadState(TSize identifier) const;
	...
	private:
		...
		void UpdateGamepadState(GamepadState* gamepad);
		...
		GamepadState newGamepadStates[4];
		GamepadState oldGamepadStates[4];
