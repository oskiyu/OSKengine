...
layout(location = 1) out mat3 outTangentMatrix;
...
void main() {
    // TAA
    const float jitterFloat = model.resolution_and_jitter.z;
    int jitter = 0;
    for (int i = 0; i < 4; i++)
        if (abs(jitterFloat - i) < 0.2)
            jitter = i;

    const vec2 resolution = model.resolution_and_jitter.xy;
    ...
    vec3 tangent = normalize(vec3(model.modelMatrix * vec4(inTangent, 0.0)));
    ...
    gl_Position = jitterMatrix * camera.projectionView * model.modelMatrix * vec4(inPosition, 1.0);
}
